...
## 2026-08-12 Standard one-to-six-player MPS P2P flow

- The separate shared test-room UI and room-service API have been removed. The existing MPS public-room create, refresh, and join controls are the only MPS entry points and retain the six-player session capacity.
- A single connected MPS host may start without a remote WebRTC peer or a `Game ready` acknowledgement. With two through six connected players, every current participant must still reach the full direct-P2P mesh and toggle `Game ready` before the Host can start. The direct-P2P bypass is scoped to MPS only; legacy Relay join-code and LAN flows keep their existing behavior.
- Every remote participant uses one `RTCPeerConnection` containing four DataChannels: unordered/unreliable snapshot and ball state, ordered/reliable combat and ball events. The four channels share the peer connection's ICE negotiation and selected transport path; they do not need four public IP addresses, ports, or UDP hole-punch operations.
- ICE or peer-connection `Closed` and every active DataChannel close now move the coordinator to `Failed`, allowing the existing reconnect path to run instead of leaving setup at `Negotiating`. The configuration remains STUN-only; static/EditMode evidence cannot prove traversal through every NAT, so Host+Guest runtime validation on the target networks remains required.
- Manual Editor validation is still required and is not claimed here: (1) MPS host-alone start, (2) two Editors complete candidate/ICE/four-channel/Game-ready/start, and (3) a six-player room refuses a seventh join while preserving the two-through-six start gate.
- Verification: Unity compiled with zero Console errors; focused MPS/signaling/mesh/failure tests passed `24/24`, and the full EditMode suite passed `185/185`. These are static and EditMode checks, not proof of cross-network NAT traversal.
